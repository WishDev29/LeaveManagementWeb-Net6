...
namespace LeaveManagement.Application.Repositories
{
    public class LeaveRequestRepository : GenericRepository<LeaveRequest>, ILeaveRequestRepository
    {
        ...
        public async Task ChangeApprovalStatus(int leaveRequestId, bool approved)
        {
            ...
            if (approved)
            {
                var allocation = await leaveAllocationRepository.GetEmployeeAllocations(leaveRequest.RequestingEmployeeId, leaveRequestId);
        ...
        public async Task<bool> CreateLeaveRequest(LeaveRequestCreateVM model)
        {
            ...
            var leaveAllocations = await leaveAllocationRepository.GetEmployeeAllocations(user.Id, model.LeaveTypeId);
            ...
            if (daysRequested > leaveAllocations.NumberOfDays)
            {
                return false;
            }
        ...
        public async Task<AdminLeaveRequestViewVM> GetAdminLeaveRequestList()
        {
            ...
            var model = new AdminLeaveRequestViewVM
            {
                ...
                PendingRequests = leaveRequests.Count(j => j.Approved == null),
                RejectedRequests = leaveRequests.Count(k => k.Approved == false),
        ...
        public Task<List<LeaveRequestVM>> GetAllAsync(string employeeId)
        {
            return context.LeaveRequests
                .Where(q => q.RequestingEmployeeId == employeeId)
                .ProjectTo<LeaveRequestVM>(configurationProvider).ToListAsync();
        }
        ...
        public async Task<EmployeeLeaveRequestViewVM> GetMyLeaveDetails()
        {
            ...
            var data = GetAllAsync(user.Id);
            var requests = mapper.Map<List<LeaveRequestVM>>(data.Result);
